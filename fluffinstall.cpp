...
int main()
{
    ...
    std::cout << "\nInstalling system...\n";
    std::system("pacstrap -C /etc/pacman.d/fluffinstall.conf -K /mnt base flufflinuxsrv-filesystem linux-lts linux-firmware linux-firmware-marvell linux-firmware-bnx2x amd-ucode arch-install-scripts intel-ucode b43-fwcutter cryptsetup ddrescue diffutils dmidecode dmraid dosfstools e2fsprogs edk2-shell efibootmgr grub ethtool exfatprogs fatresize fsarchiver gpart git gptfdisk hdparm less libusb-compat lsscsi lvm2 man-db man-pages mdadm memtest86+-efi mkinitcpio modemmanager mtools nano nfs-utils nmap ntfs-3g nvme-cli open-iscsi openssh partclone parted networkmanager networkmanager-openvpn pv rp-pppoe rsync sdparm sg3_utils smartmontools squashfs-tools sudo systemd-resolvconf tcpdump testdisk tmux tpm2-tools tpm2-tss udftools usb_modeswitch usbutils vim wireless-regdb wpa_supplicant wvdial xfsprogs zsh grml-zsh-config-flufflinux fastfetch htop ttf-liberation thermald base-devel btop traceroute edk2-ovmf ttf-dejavu ttf-droid ttf-hack iperf iftop pacman-contrib flufflinux-hooks");
